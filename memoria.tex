...
A-frame~\cite{a} es la tecnología mas importante de este proyecto, ya que es la base fundamental de este. A-frame es un framework creado para la construcción de experiencias en realidad virtual, basado en HTML, por lo que como hemos dicho antes, es un leguaje bastane sencillo e intuitivo. una de las caracteristicas claves de A-frame es el framwork entidad-componente para three.js.

Esta tecnología fue desarrollada por el euipo de mozilla VR durante de 2015. El objetivo principal de estos era permitir crear dichas escenas sirectamente en HTML sin conocer WebGL. A parte de esta, también usa otras tecnologías anteriormente explicadas como son WebXR y JavaScript.
...
\begin{itemize}
    \item la inclusion de las dependencias de Aframe en el elemento Head, nos permite usar todos los elementos de esta tecnlogia
    \item La creacion de las diferentes figuras mediante sus respectivas etiequetas
    \item Modificación de ciertos aspectos de las figturas mediante los atributos
\end{itemize}

Profundizanod un poco  mas sobre entidad y componente y como conclusion, Una entidad es el objento en sí creado emdiante HTML y el componente es el corpotamiento que le podemos asginar a dicha figura, estos se realizan el JavaScript. 

Estos componentes siguen una escrtura estandarizada, algunos son el schema, propiedas del componente, init, parte del componente que se ejectuta al iniciar el componente. La manera correcta de incluir un componente a la escena es añadirselo al elemento como si de un atributo se tratara.
